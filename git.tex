...
\item You are now in \verb+testing+. Create a new file
  \verb+experimental+ and add some lines to it. Commit. Add some lines
  to \verb+mainFile+ too. Commit.
\item See the history of your files using \verb+git log+.
\item Go back to the \emph{master} branch: \verb+git checkout
  master+. 
...
development. Any commits you make in \verb+testing+ are not visible in
\verb+master+. If at a later point you want to merge both 
