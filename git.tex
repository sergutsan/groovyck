...
created a repository in GitHub called MyCurrencyExchange. 
...
the history of your code and understand every step. The cost of
...
that you write. Committing every single line is probably too much,
too, unless those lines are really special. Do not spend more time
...
If you are not sure whether you have made a lot of changes worth of
being committed, you can use \verb+git diff+. Another possibility,
less verbose, is to use \verb+git status+. The latter will tell you
...
repository fairly often. Some people say \emph{push early, push
  often}, but you do not need to do this every ten minutes. Sometimes
...
or standing up to grab some lunch).
...
it is not uncommon for user to use email addresses that are perfectly
...
The way to tell Git to ignore these files is by adding them to a file
