...
\verb+procedures+. Methods that do return a value are sometimes called
\verb+functions+ by analogy with mathematical functions.

\subsubsection{Beware of parameters of complex types!}
\label{beware}

In the former section we have seen that methods make a copy of their
...
As you can see, the integer (a simple type) is changed inside the
method but this change is forgotten as soon as you leave the
method. The point is also modified inside the method, in two different
ways: first, the value of one of its coordinates is changed; then, the
point itself is set to \verb+null+. When we come out of the method,
...
objects those boxes are pointing to are changed, those changes stay
even after you leave the method. 

\textbf{Note. } In books, you may find that passing values to a
methods as parameters is called \emph{pass parameters by value}, while
passing pointers is called \emph{pass parameters by reference}. The
...
reference do. This is because the method only copies the ``box'', the
pointer, not the object it points to. 

\subsubsection{Exercise}
\label{sec:exerciseff}

Draw a diagram of all the pointers involved in the example above and
make sure that you understand all the things that are copied at every
point. 
...
member methods. You already know some example like \verb+length()+,
...
class (e.g.+\verb+str.length()+), in the same way that fields can only
...
keyword, \verb+this+, which means ``this object we are not in''. It is
...
\verb+x+ field of \emph{this} object (\emph{point}) I am in now and
...
return \verb+void+, nothing. They are sometimes called
procedures. Even if they do not return anything, they can be useful
...
age of the tamagotchi. This is called a side effect because it is not
returned.
...
also alter the flow of execution. 

When a method is called, the next line being executed is the first
line in the method, and then the flow continues in the methods as in a
